...
function Review({ title, body, author, rating, className, ...props }) {
  ...
  return (
    <figure
      className={clsx(
        "animate-fade-in rounded-3xl border border-slate-400 p-6 opacity-0 shadow-md shadow-gray-900/5",
        className,
      )}
    ...
    >
      <blockquote className="text-gray-900">
        ...
        <p className="mt-4 text-lg font-semibold leading-6 text-slate-200 before:content-['“'] after:content-['”']">
          {title}
        </p>
        <p className="mt-3 leading-7 text-slate-300">{body}</p>
      </blockquote>
      <figcaption className="mt-3 text-sm text-slate-400 before:content-['–_']">
        {author}
      </figcaption>
    </figure>
  );
...
function ReviewGrid() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-[#0b0b16]" />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-[#0b0b16]" />
    </div>
  );
}
...
export function Reviews() {
  return (
    <section
      ...
      className="pb-16 pt-20 sm:pb-24 sm:pt-16"
    >
      <Container>
        <h2
          id="reviews-title"
          className="text-3xl font-medium tracking-tight sm:text-center"
        >
          What people are saying
        </h2>
        <p className="mt-2 text-lg text-slate-400 sm:text-center">
          Dozens of entrepreneurs, influencers, and other engineers have worked with me
        </p>
        <ReviewGrid />
      </Container>
    </section>
